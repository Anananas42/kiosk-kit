...
export default function Confirm({ buyer, buyerLabel, category, item, isPreorder, noDeliveryDays, onConfirm, onBack, isSending, error, onErrorDismiss, locale, currency }: ConfirmProps) {
  const t = useT();
        ...
        <div className="screen-body screen-body--preorder-confirm">
          <div className="preorder-confirm-summary">
            ...
            {unitPrice > 0 && (
              <>
                <span className="preorder-confirm-summary__sep">&middot;</span>
                <span>{t('confirm.perUnit', { price: formatCurrency(unitPrice, locale, currency) })}</span>
              </>
            )}
          </div>
          ...
          {deliveryDate && (
            <div className="delivery-notice">
              ...
              {existingQty !== null && existingQty > 0 && <> &middot; {t('confirm.alreadyOrdered', { count: existingQty })}</>}
            </div>
          )}
              ...
              {canStorno && (
                confirmingStorno ? (
                  <div className="confirm-storno-gate">
                    <span className="confirm-storno-gate__label">{t('confirm.removeQty', { qty: stornoQty })}</span>
