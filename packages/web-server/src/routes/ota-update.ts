...
export function otaUpdateRoutes(db: Db) {
  const app = new Hono<AuthEnv>();

  // POST /api/devices/:id/ota/install
  app.post("/:id/ota/install", async (c) => {
    ...
    // Check for active ota_push — can't install while pushing
    const activePush = await getLatestOperation(db, device.id, OperationType.OtaPush);
    ...
    try {
      await fetchDeviceProxy(device, "/api/trpc/admin.ota.install", {
        method: "POST",
  ...
  // POST /api/devices/:id/ota/cancel
  app.post("/:id/ota/cancel", async (c) => {
    const user = c.get("user");
    const deviceId = c.req.param("id");
    ...
    try {
      await fetchDeviceProxy(device, "/api/trpc/admin.ota.cancelUpload", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
        signal: AbortSignal.timeout(DEVICE_TIMEOUT_MS),
    ...
    // Fail any active ota_push operation
    const activePush = await getLatestOperation(db, device.id, OperationType.OtaPush);
    if (activePush?.status === OperationStatus.InProgress) {
  ...
  // POST /api/devices/:id/ota/rollback
  app.post("/:id/ota/rollback", async (c) => {
    const user = c.get("user");
    const deviceId = c.req.param("id");
    ...
    const { operation, isNew } = await startOperation(db, {
      deviceId: device.id,
      type: OperationType.OtaRollback,
      staleThresholdMs: UPDATE_STALE_OP_MS,
    });
    ...
    try {
      const res = await fetchDeviceProxy(device, "/api/trpc/admin.ota.rollback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
        signal: AbortSignal.timeout(DEVICE_TIMEOUT_MS),
  ...
  // GET /api/devices/:id/ota/status
  app.get("/:id/ota/status", async (c) => {
    const user = c.get("user");
    const deviceId = c.req.param("id");
    ...
    try {
      const res = await fetchDeviceProxy(device, "/api/trpc/admin.ota.status", {
        method: "GET",
        signal: AbortSignal.timeout(DEVICE_TIMEOUT_MS),
      });
    ...
    // Fallback: latest OTA operation from DB
    const latestPush = await getLatestOperation(db, device.id, OperationType.OtaPush);
    const latestInstall = await getLatestOperation(db, device.id, OperationType.OtaInstall);
  ...
  // PATCH /api/devices/:id/ota/complete
  app.patch("/:id/ota/complete", async (c) => {
    const user = c.get("user");
    const deviceId = c.req.param("id");
    ...
    const latest = await getLatestOperation(db, device.id, OperationType.OtaInstall);
